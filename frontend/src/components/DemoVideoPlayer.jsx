...
import { LayoutDashboard, Upload, Zap, Lightbulb, Sparkles, History, Play, Pause, RotateCcw } from 'lucide-react';
...
export default function DemoVideoPlayer() {
    ...
    return (
        <div className="relative w-full h-full bg-navy-950 flex flex-col overflow-hidden select-none">
            {/* Background Animations to simulate motion graphics */}
            ...
            <div className="flex-1 relative flex flex-col items-center justify-center p-8 text-center z-10">
                <AnimatePresence mode="wait">
                    ...
                    >
                        <div className={`w-20 h-20 sm:w-28 sm:h-28 rounded-3xl bg-gradient-to-br ${scene.color} p-0.5 mb-8 shadow-2xl`}>
            ...
            <div className="h-16 bg-navy-900/80 backdrop-blur-md border-t border-white/10 flex items-center px-6 z-20 gap-4">
                <button
                    onClick={() => isPlaying ? setIsPlaying(false) : (currentIndex === SCENES.length - 1 ? handleRestart() : setIsPlaying(true))}
                ...
                <div className="flex-1 flex gap-2 h-1.5 items-center">
                    {SCENES.map((_, idx) => (
                        <div key={idx} className="flex-1 h-full bg-navy-950 rounded-full overflow-hidden relative cursor-pointer" onClick={() => { setCurrentIndex(idx); setProgress(0); setIsPlaying(true); }}>
                            {(idx < currentIndex) && <div className="absolute inset-0 bg-cyan-500" />}
                            {(idx === currentIndex) && (
                                <div className="absolute top-0 left-0 bottom-0 bg-cyan-400 shadow-[0_0_10px_rgba(34,211,238,0.8)]" style={{ width: `${progress}%` }} />
                            )}
                        </div>
                    ))}
                </div>
            ...
            {!isPlaying && currentIndex === SCENES.length - 1 && (
                <div className="absolute inset-0 z-30 flex items-center justify-center bg-navy-950/60 backdrop-blur-sm">
                    <button onClick={handleRestart} className="btn-primary py-4 px-10 shadow-glow-cyan flex items-center gap-3">
                        <RotateCcw size={20} />
                        Restart Demo Tour
                    </button>
                </div>
