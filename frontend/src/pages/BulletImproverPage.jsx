...
import { Sparkles, ArrowRight, Copy, CheckCheck, Loader2, RotateCcw, ChevronRight } from 'lucide-react';
...
function DiffHighlight({ original, improved }) {
    ...
    return (
        <p className="text-sm leading-7 text-nexus-text">
            {impWords.map((word, i) => {
                const isNew = !origWords.includes(word.replace(/[^a-z0-9%$]/gi, '').toLowerCase());
                return (
                    <span key={i}>
                        <span className={isNew ? 'bg-emerald-500/20 text-emerald-300 px-0.5 rounded' : ''}>
                            {word}
                        </span>
                        {' '}
                    </span>
...
function CopyBtn({ text }) {
    ...
    const go = () => { navigator.clipboard.writeText(text); setC(true); setTimeout(() => setC(false), 2000); };
    return (
        <button onClick={go} className={`flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-lg border transition-all
...
export default function BulletImproverPage() {
    ...
    // Pre-populate from star_bullets in analysis if available
    const starBullets = analysisResult?.star_bullets || {};
    const preloaded = Object.entries(starBullets).slice(0, 4);

    const improve = async () => {
        if (!bullet.trim()) { setError('Please enter a bullet point to improve.'); return; }
    ...
    return (
        <PageWrapper>
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
                <span className="badge-cyan mb-4 inline-flex"><Sparkles size={11} /> STAR Bullet Engine</span>
                ...
                <p className="text-nexus-muted text-sm mt-1">
                    Transform vague bullets into STAR-method impact statements with quantifiable metrics.
                </p>
            </motion.div>
            ...
            <div className="grid lg:grid-cols-2 gap-6">
                ...
                >
                    ...
                    <div className="glass-card-sm p-4">
                        <p className="text-xs font-semibold text-nexus-muted mb-3 uppercase tracking-wide">Try an example</p>
                        <div className="space-y-1.5">
                            {EXAMPLES.map((ex, i) => (
                                <button key={i} onClick={() => { setBullet(ex); setResult(null); setError(null); }}
                                ...
                                >
                                    <ChevronRight size={11} className="text-nexus-cyan shrink-0" />
                                    {ex}
                                </button>
                            ))}
                        </div>
                    ...
                    <AnimatePresence mode="wait">
                        {result ? (
                            ...
                            >
                                ...
                                <div className="glass-card p-5 border-emerald-500/20">
                                    <div className="flex items-center justify-between mb-3">
                                        <span className="badge-emerald text-[11px]">✦ AI Enhanced</span>
                                        <CopyBtn text={result.improved || ''} />
                                    </div>
                                    <DiffHighlight original={bullet} improved={result.improved || ''} />
                                ...
                                <div className="glass-card-sm p-4 opacity-60">
                                    <p className="text-[10px] text-nexus-muted mb-2 uppercase tracking-wide font-semibold">Original</p>
                                    <p className="text-xs text-nexus-muted leading-relaxed">{bullet}</p>
                                </div>

                                {/* Metrics breakdown */}
                                {result.metrics && (
                                    <div className="glass-card-sm p-4">
                                        <p className="text-xs font-semibold text-nexus-muted mb-3 uppercase tracking-wide">STAR Analysis</p>
                                        {Object.entries(result.metrics).map(([k, v]) => (
                                            <div key={k} className="flex justify-between items-center py-1.5 border-b border-white/5 last:border-0">
                                                <span className="text-xs text-nexus-muted capitalize">{k}</span>
                                                <span className="text-xs text-nexus-text font-medium">{v}</span>
                                            </div>
                                        ))}
                                    </div>
                                )}
                            ...
                            >
                                ...
                                <p className="text-nexus-muted text-sm">Your improved bullet will appear here</p>
                                <p className="text-nexus-muted/50 text-xs mt-1">Powered by STAR-method AI analysis</p>
                            </motion.div>
                        )}
                    </AnimatePresence>
                            ...
                            <div className="space-y-3">
                                {preloaded.map(([orig, imp], i) => (
                                    <div key={i} className="p-3 rounded-xl bg-white/3 border border-white/5">
                                        <p className="text-[10px] text-nexus-muted mb-1">Original</p>
                                        <p className="text-xs text-nexus-muted line-through opacity-60 mb-2">{orig}</p>
                                        <p className="text-[10px] text-emerald-400 mb-1">Improved</p>
                                        <p className="text-xs text-nexus-text leading-relaxed">{imp}</p>
                                    </div>
                                ))}
                            </div>
                        </div>
