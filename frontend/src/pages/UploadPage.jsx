...
export default function UploadPage() {
    ...
    const removeFile = () => { setLocalFile(null); setFile(null); clearError(); };

    const handleAnalyze = async () => {
        if (!localFile) { setError('Please upload a resume first.'); return; }
        setLoading(true);
        setError(null);
        try {
            ...
            setError(err?.response?.data?.error || 'Analysis failed. Make sure the backend is running on port 4000.');
        } finally {
            setLoading(false);
        }
    };
            ...
            <div className="max-w-2xl mx-auto">
                ...
                >
                    ...
                    <p className="text-nexus-muted text-sm leading-relaxed">
                        Your resume is analyzed locally and never stored. AES-256 encrypted in transit.
                    </p>
                </motion.div>
              ...
              ${dragging
                            ...
                            {localFile ? (
                                ...
                                >
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <p className="font-semibold text-nexus-text truncate">{localFile.name}</p>
                                        <p className="text-xs text-nexus-muted mt-0.5">
                                            {(localFile.size / 1024).toFixed(0)} KB · {localFile.type.includes('pdf') ? 'PDF' : 'DOCX'}
                                        </p>
                                    </div>
                                ...
                                >
                                    <div className={`w-16 h-16 rounded-2xl mx-auto mb-4 flex items-center justify-center border transition-all ${dragging ? 'border-cyan-400 bg-cyan-500/15' : 'border-white/10 bg-white/5'
                                        }`}>
                                        <Upload size={26} className={`transition-colors ${dragging ? 'text-cyan-400' : 'text-nexus-muted'}`} />
                                    </div>
                                    <p className="font-semibold text-nexus-text mb-1">
                                        {dragging ? 'Drop it here!' : 'Drag & drop your resume'}
                                    </p>
                                    <p className="text-sm text-nexus-muted mb-4">or click to browse files</p>
                                    <div className="flex gap-2 justify-center">
                                        <span className="badge bg-white/5 text-nexus-muted border-white/10 text-[10px]">PDF</span>
                                        <span className="badge bg-white/5 text-nexus-muted border-white/10 text-[10px]">DOCX</span>
                                        <span className="badge bg-white/5 text-nexus-muted border-white/10 text-[10px]">Max 5MB</span>
                                    </div>
                                </motion.div>
                            )}
                        </AnimatePresence>
                    ...
                    <div>
                        ...
                        <div className="relative">
                            ...
                            >
                                {JOB_ROLES.map(r => (
                                    <option key={r} value={r} className="bg-navy-800">{r}</option>
                                ))}
                            </select>
                            <ChevronDown size={16} className="absolute right-3 top-1/2 -translate-y-1/2 text-nexus-muted pointer-events-none" />
                        </div>
                    ...
                    <div className="flex items-center justify-center gap-6 pt-2">
                        {['End-to-end encrypted', 'Auto-deleted after analysis', 'GDPR compliant'].map(t => (
                            <span key={t} className="flex items-center gap-1.5 text-[11px] text-nexus-muted">
