    ...
    return (
        <PageWrapper>
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
                <span className="badge-indigo mb-4 inline-flex"><History size={11} /> Analysis Log</span>
            ...
            {history.length > 0 && (
                ...
                >
                    {[
                        { icon: FileText, label: 'Total Analyses', value: history.length, color: 'text-cyan-400' },
                        { icon: TrendingUp, label: 'Avg Score', value: Math.round(history.reduce((a, h) => a + (h.overall_score || 0), 0) / history.length), color: 'text-indigo-400' },
                        { icon: Award, label: 'Best Score', value: Math.max(...history.map(h => h.overall_score || 0)), color: 'text-emerald-400' },
                        { icon: Calendar, label: 'Days Active', value: Math.ceil((new Date() - new Date(history[history.length - 1]?.date || Date.now())) / 86400000) || 1, color: 'text-amber-400' },
            ...
            {loading ? (
                ...
                <div className="glass-card p-16 text-center">
                    <History size={40} className="text-nexus-muted mx-auto mb-4 opacity-40" />
                    <p className="text-nexus-muted">No analysis history yet. Upload a resume to get started.</p>
                </div>
            ) : (
                <motion.div variants={containerVariants} initial="hidden" animate="show">
                    ...
                    <div className="hidden md:block glass-card overflow-hidden">
                        <table className="w-full">
                            ...
                            <tbody>
                                {history.map((item, i) => (
                                    <motion.tr key={item.id || i} variants={rowVariants}
                                    ...
                                    >
                                        <td className="px-5 py-4">
                                            <div className="flex items-center gap-2.5">
                                                ...
                                                <div>
                                                    <p className="text-sm font-semibold text-nexus-text truncate max-w-[140px]">{item.filename}</p>
                                                    <p className="text-xs text-nexus-muted">{item.job_role}</p>
                                                </div>
                                            </div>
                                        </td>
                                        <td className="px-5 py-4">
                                            <p className="text-sm text-nexus-muted">{new Date(item.date).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</p>
                                        </td>
                                        <td className="px-5 py-4">
                                        ...
                                        <td className="px-5 py-4">
                                            <Sparkline data={item.score_history || [item.overall_score]} />
                                        </td>
                                        <td className="px-5 py-4">
                                            <div className="flex flex-wrap gap-1 max-w-[160px]">
                                                {(item.keywords_missing || []).slice(0, 3).map(k => (
                                                    <span key={k} className="px-1.5 py-0.5 rounded text-[10px] bg-rose-500/10 text-rose-300 border border-rose-500/15">{k}</span>
                                                ))}
                                        ...
                                        <td className="px-5 py-4">
                                            <span className={`badge text-[10px] border ${scoreBg(item.overall_score)} ${scoreColor(item.overall_score)}`}>
                    ...
                    <div className="md:hidden space-y-4">
                        {history.map((item, i) => (
                            <motion.div key={item.id || i} variants={rowVariants} className="glass-card p-5">
                                <div className="flex items-start justify-between mb-3">
                                    <div>
                                        <p className="font-semibold text-nexus-text">{item.filename}</p>
                                        <p className="text-xs text-nexus-muted">{item.job_role}</p>
                                    </div>
                                    <span className={`text-2xl font-black ${scoreColor(item.overall_score)}`}>{item.overall_score}</span>
                                </div>
